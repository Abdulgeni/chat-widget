...
export function listWidgetConfigs() {
  const rows = db.prepare('SELECT * FROM widget_configs').all();
  return rows.map((row) => ({
    appId: row.app_id,
    allowedDomains: JSON.parse(row.allowed_domains),
    theme: JSON.parse(row.theme),
...
export function deleteWidgetConfig(appId) {
  db.prepare('DELETE FROM widget_configs WHERE app_id = ?').run(appId);
}